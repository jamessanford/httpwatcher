...
```bash
sudo bpftool prog load bpf/uprobe.bpf.o /sys/fs/bpf/httpwatcher_test type uprobe
sudo bpftool prog show pinned /sys/fs/bpf/httpwatcher_test
sudo rm /sys/fs/bpf/httpwatcher_test
```
...
When httpwatcher's `Attach(pid)` is called, the library reads the target binary's
